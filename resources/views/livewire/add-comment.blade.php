<div
    x-data="{ isOpen: false }"
    class="relative"
    x-on:idea-was-commented.window="isOpen = false;"
...
>
    ...
    <div class="absolute z-10 w-64 mt-2 text-sm font-semibold text-left bg-white md:w-104 shadow-dialog rounded-xl"
        ...
        @auth
                <form wire:submit.prevent='postComment' action="#" class="px-4 py-6 space-y-4">
                    <div>
                        <textarea x-ref="comment" wire:model='body' name="post_comment" id="post_comment" cols="30" rows="4"
                            class="w-full px-4 py-2 text-sm placeholder-gray-400 bg-gray-100 border border-gray-200 rounded-xl"
